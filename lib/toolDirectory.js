...
// Round 413 — union of DEFAULT_TOOL_DIRECTORY's hardcoded bucket keys AND
// whatever bucket keys exist in the saved row, not just the hardcoded ones.
// Needed once the Tools Directory editor can create brand-new page groups
// at runtime (see app/tool-directory/page.js's addPageGroup) — those
// buckets have no DEFAULT_TOOL_DIRECTORY entry to merge onto, so they'd
// silently vanish on next load if this only ever walked the hardcoded
// keys. A bucket with no `def` is trusted as-is (already the full saved
// shape — team/pageLabel/route/tools).
export function mergeToolDirectory(saved, milestoneChartLinks) {
  const merged = {};
  const allKeys = new Set([...Object.keys(DEFAULT_TOOL_DIRECTORY), ...Object.keys(saved || {})]);
  allKeys.forEach((bucketKey) => {
    const def = DEFAULT_TOOL_DIRECTORY[bucketKey];
    const savedBucket = saved?.[bucketKey];
    if (!def) {
      if (savedBucket) merged[bucketKey] = savedBucket;
      return;
    }
...
// Round 413 — "allow me to fix on all team (currently only OPS; should be
// AR, Marketing and legal as well)". Was previously derived from
// DEFAULT_TOOL_DIRECTORY's own bucket.team values, which meant a team with
// zero hardcoded buckets (every team except OPS) never even showed up as a
// tab — there was nowhere to click "+ Add Page Group" for it. Now a fixed
// list, independent of which buckets happen to exist yet, so AR/
// Marketing/Legal show up (empty, with an add-a-page-group prompt) even
// before a dev has created their first bucket. Design intentionally left
// out — not asked for, and Design's one ticket type/no workstations gives
// it little use for an external-tools directory today; add it here the
// same way if that changes.
export const DIRECTORY_TEAMS = ["OPS", "AR", "Marketing", "Legal"];

export const TEAM_LABELS = { OPS: "Operation / OPS", AR: "AR", Marketing: "Marketing", Legal: "Legal" };

// Round 413 — id helpers for tools/buckets added through the editor
// (app/tool-directory/page.js's "+ Add Tool" / "+ Add Page Group"), rather
// than seeded here in code. Slug-then-suffix so a human-entered label still
// reads sensibly as a key/object property, with the suffix only there to
// avoid a collision if two things get the same label.
function slugify(label) {
  return (label || "tool").toLowerCase().trim().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "tool";
}
export function makeBucketKey(pageLabel) {
  return `${slugify(pageLabel)}_${Date.now().toString(36).slice(-5)}`;
}
export function makeToolKey(label) {
  return `${slugify(label)}_${Date.now().toString(36).slice(-5)}${Math.random().toString(36).slice(2, 4)}`;
}
