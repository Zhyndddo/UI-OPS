...
export const TICKET_CONFIGS = {
  ...
  khac: {
    ...
    fields: [
      ...
      // Round 325 — "personal ticket aka self note", per explicit request:
      // a Khác ticket can now be flagged as a private task for just its
      // own requester instead of the normal fully-shared ("Tất cả") view.
      // NewTicketPage.js auto-assigns the requester as PIC when this is
      // checked (so it counts toward their own Task Table row the same
      // way any other PIC'd ticket does — see app/task-table/page.js's
      // "khac" column), and TicketListPage.js filters every fetch so no
      // one but the requester ever sees it in the list (see
      // selfTaskVisibilityFilter). Excluded from the list's data columns
      // (lib/TicketListPage.js's COMPUTED_REPLACES) since it's a
      // visibility flag, not something to display/edit as a field value —
      // shown instead as a small "🔒 Personal" badge.
      { key: "selfTask", label: "Self Task — only visible to you", type: "checkbox", defaultValue: false, helpText: "Personal note/reminder. Assigns you as PIC and hides this ticket from everyone else." },
    ],
  },
