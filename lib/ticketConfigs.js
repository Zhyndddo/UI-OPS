...
export const TICKET_CONFIGS = {
  ...
  // list+form system.
  phai_sinh: {
    // Deadline defaults to whatever's typed into the releaseDate field,
    // until the requester picks a deadline themselves — see NewTicketPage's
    // defaultDeadlineFrom handling below.
    defaultDeadlineFrom: "releaseDate",
    // No releaseFieldMap here anymore — Tên Bài is now a plain hand-typed
    // field (the derivative's own title, which is usually NOT the same
    // text as the original release it's derived from). The link back to
    // the original product is the separate Related DID field below,
    // which has its own live-search combobox (see RelatedDidField).
    label: "Phái Sinh",
    requesterTeam: "AR",
    executorTeam: "OPS",
