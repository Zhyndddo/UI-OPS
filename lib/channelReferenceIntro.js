...
//
// Returns { title, text, canvaUrl, sheetUrl } (all "" if unset/
// unreachable/unparseable) — every call site treats blank as "nothing to
// show," so a missing or malformed row fails safely open to just not
// rendering the block, same fail-open shape readMagicLinkThemeLock uses.
export async function readChannelReferenceIntro(supabase) {
  if (!supabase) return { title: "", text: "", canvaUrl: "", sheetUrl: "" };
  try {
    ...
    return { title: "", text: "", canvaUrl: "", sheetUrl: "" };
  }
}

// Serializes { title, text, canvaUrl, sheetUrl } for writing back into
