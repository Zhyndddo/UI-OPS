...
// Round 329 — BUG FIX: global_settings.value is a plain `text` column in
// production (see sql/reference/prod_schema_clean.sql), not jsonb — same
// bug just fixed in lib/channelReferenceIntro.js. Reading `data.value.theme`
// off a raw string always silently returned undefined, so this lock never
// actually read back as set (the CSS specificity fix earlier was real and
// still needed, but couldn't matter until this read/write also worked).
// Now parses/serializes explicitly, matching the rest of the app's
// global_settings usage (see lib/troGiaBooking.js).
...
// Serializes the lock setting for writing into global_settings.value.
// `next` is a valid theme string, or falsy to clear the lock.
export function serializeMagicLinkThemeLock(next) {
  return JSON.stringify(next ? { theme: next } : {});
}
