"use client";

import { usePathname } from "next/navigation";
import Link from "next/link";
import { useEffect, useState } from "react";
import { supabase } from "./supabaseClient";
import { useTheme } from "./ThemeContext";
import { useAuth } from "./AuthContext";
import { canRunPackageSimulator, canSendSecretMessages, isAdminOrAbove } from "./permissions";
import { loadMyActiveSecretMessages, getHiddenSecretMessageIds } from "./secretMessages";
// Round 404 — System Messages (auto-generated, not admin-composed — see
// lib/systemMessages.js's header comment for why this is a separate
// mechanism from Secret Messages above) plus the two daily generators
// that feed it.
import {
  loadMyActiveSystemMessages, getHiddenSystemMessageIds, hideSystemMessageForProfile,
  getSnoozedSystemMessageIds, snoozeSystemMessageForProfile, clearSnoozedSystemMessages,
} from "./systemMessages";
import { rollForwardIfNeeded, persistRollForward } from "./weeklyTasks";
import { ensureBoSungDataDigest } from "./boSungDataDigest";
// Round 405 — "count if the user is afk for a long time (more than 4
// hours) then count them as first login on next activity" — see
// lib/afkTracker.js's header comment for the full reasoning.
// Round 406 — also used with a longer, 24h threshold to reset the new
// "hide until next login" snooze below.
import { watchForAfkReturn, DEFAULT_AFK_THRESHOLD_MS } from "./afkTracker";

const SNOOZE_RESET_THRESHOLD_MS = 24 * 60 * 60 * 1000; // Round 406 — 24h

const NAV = [
  { label: "Dashboard", href: "/releases" },
  { label: "Workstation", href: "/workstation" },
  { label: "Tickets", href: "/tickets" },
  // Round 56 added "Report" (coherent read across releases/booking/package
  // data — tables + charts). Round 57 merged the old standalone "Summary"
  // worklist (per-team not-done counts) into Report as a second tab ("Team
  // Worklist"), so there's just one nav entry now. /summary still works —
  // it redirects to /report — but isn't linked from the sidebar anymore.
  // Round 255 — devOnly per explicit request ("lock the report sidebar
  // item for dev role only"). Same convention every other role gate in
  // this app already uses (there's no real access control anywhere — see
  // sql/reference/prod_schema_clean.sql, no RLS at all — so "hide the nav
  // link" IS the actual gate, same as Tools/Task Table etc. being visible
  // to everyone by design); app/report/page.js itself has no matching
  // redirect guard, consistent with that same convention.
  // Round 273 — widened from dev-only to admin-and-up (isAdminOrAbove,
  // lib/permissions.js's 4-tier model — admin AND dev both pass now), per
  // explicit request ("add admin role to the sidebar item report").
  { label: "Report", href: "/report", adminOnly: true },
  // Round 82 item 1 — read-only compiled overview of every workstation +
  // ticket type, one row each with a clickable row count.
  { label: "Task Table", href: "/task-table" },
  // Round 172 item 3 — kanban calendar of releases (last/this/next week,
  // grouped by pipeline stage), per explicit request.
  { label: "Calendar", href: "/calendar" },
  // Round 155 item 1 — compiled external-tools directory (team-per-tab,
  // grouped by page). Visible to everyone (view-only) same as every NAV
  // entry above — editing is dev-gated INSIDE the page itself, not by
  // hiding the sidebar link. Desktop-only content, but the link itself
  // still shows on mobile per explicit request ("mobile only get the
  // sidebar item") — see app/tool-directory/page.js's mobile branch.
  { label: "Tools", href: "/tool-directory" },
  // Round 408 — item 2 from the original Round 404 request: a sidebar
  // item for VCPMC's catalog, "under a different name" per explicit
  // request — "Royalty Catalog" rather than naming VCPMC directly. Visible
  // to everyone (view-only reference, same convention as Reference/Tools)
  // — the Reports review sub-tab inside the page itself is what's
  // admin-gated (isAdminOrAbove), not this link.
  { label: "Royalty Catalog", href: "/royalty-catalog" },
];
// Round 407 — Round 85's own comment said to delete this sidebar entry
// once results were reported out; per explicit request ("Bảng Khảo Sát
// Nhanh Chuyến Đi Chơi also remove this sidebar item") that's now done.
// The page itself (app/team-building-survey/page.js), its questions
// config (lib/teamBuildingSurveyQuestions.js), and the
// team_building_survey_responses table are all deliberately left in
// place — only the sidebar link is gone, same scope as the original
// request (removing the nav item, not the underlying feature/data);
// still reachable by direct URL for anyone who needs the results.
// Pulled out of the Tickets switcher onto the main sidebar directly, per
// request — "Khác" (the shared catch-all ticket type every team can use)
// gets its own top-level shortcut instead of being one option among many
// once you're already inside Tickets. Its label is admin-editable (Config
// -> Sidebar Label, dev-only) via app_settings.khac_sidebar_label — this
// is just the fallback shown before that setting loads or if it's never
// been set. See DEFAULT_KHAC_LABEL below.
//
// Position: always last, below everything else the current user has in
// their sidebar (including AR_NAV, which it used to sit above) — per
// explicit request. Numbers are no longer hardcoded per item; they're
// assigned by final array position in navItems below, so Khác always gets
// whatever number is last regardless of whether AR_NAV is shown.
const KHAC_HREF = "/tickets/khac";
const DEFAULT_KHAC_LABEL = "Cứu mạng Zhyn ơi";
// New Release, Tools, Artists, and Labels are deliberately not in the
// main nav for everyone else — Artists/Labels live in Reference (sidebar
// bottom) already, no need for a duplicate top-level entry. AR is the
// exception: they're the team that actually owns/maintains these two
// tables day to day, so they get direct top-level shortcuts too (in
// addition to, not instead of, the Reference entries everyone else uses).
const AR_NAV = [
  { label: "Artist List", href: "/artists" },
  { label: "Label List", href: "/labels" },
];
// Round 153 — item 3: new top-level shortcut straight to the New Release
// creation form (/new-release), dev-only per explicit request. Kept as
// its own small array (like AR_NAV) rather than folded into the shared
// NAV list, since it's conditionally spliced in below based on role.
const DEV_NAV = [{ label: "NEW REQUEST", href: "/new-release" }];

export const SIDEBAR_WIDTH = 250;

// Round 87 — open/onClose are only meaningful on mobile (see AppShell.js):
// desktop always passes open=true + mobile=false, so the transform below
// stays translateX(0), no backdrop/close-button render, and nothing here
// changes visually from before. onClose is always passed (even on desktop,
// where calling it is a harmless no-op — AppShell forces open=true
// regardless of its internal sidebarOpen state) so every Link below can
// close the drawer after navigating without extra plumbing.
export default function Sidebar({ open = true, onClose, mobile = false }) {
  const pathname = usePathname();
  const { theme, toggleTheme } = useTheme();
  const { profile } = useAuth();
  const [totalReleases, setTotalReleases] = useState(null);
  const [khacLabel, setKhacLabel] = useState(DEFAULT_KHAC_LABEL);
  const [secretMessages, setSecretMessages] = useState([]);
  const [secretPopup, setSecretPopup] = useState(null);
  // Round 404 — System Messages: same shape as the Secret Messages state
  // just above, deliberately kept as a separate pair of variables/popup
  // rather than merged in, so this round's new auto-generated mechanism
  // can't accidentally regress the existing admin-composed one.
  const [systemMessages, setSystemMessages] = useState([]);
  const [systemPopup, setSystemPopup] = useState(null);
  const showArNav = profile?.segment === "AR" || profile?.role === "dev";
  const showPackageRunner = canRunPackageSimulator(profile);
  const isDev = profile?.role === "dev";
  const isAdminUp = isAdminOrAbove(profile);
  // Round 306 — Secret Messages moved out of Config onto this sidebar link
  // directly (was dev-only compose inside Config; now any admin can send,
  // scoped to their own team — see lib/permissions.js). The link used to
  // render ONLY while there was an unread message (see the comment further
  // down); it now also renders unconditionally for anyone who can send,
  // so they have a way to reach the Send tab even with an empty inbox.
  const canSendSecret = canSendSecretMessages(profile);
  // Khác goes last, after AR_NAV/DEV_NAV — see the comment above
  // AR_NAV/KHAC_HREF. DEV_NAV placed after AR_NAV, same "extra shortcut
  // for a specific audience, above Khác" pattern. devOnly/adminOnly items
  // (just Report, Round 255/273) are dropped for anyone else before
  // numbering, so numbers stay contiguous instead of skipping.
  const navItems = [...NAV, ...(showArNav ? AR_NAV : []), ...(isDev ? DEV_NAV : []), { label: khacLabel, href: KHAC_HREF }]
    .filter((item) => !item.devOnly || isDev)
    .filter((item) => !item.adminOnly || isAdminUp)
    .map((item, i) => ({ ...item, num: String(i + 1).padStart(2, "0") }));

  useEffect(() => {
    if (!supabase) return;
    supabase
      .from("app_settings")
      .select("value")
      .eq("key", "khac_sidebar_label")
      .maybeSingle()
      .then(({ data }) => { if (typeof data?.value === "string" && data.value.trim()) setKhacLabel(data.value); });
  }, []);

  useEffect(() => {
    if (!supabase) return;
    supabase
      .from("releases")
      .select("id", { count: "exact", head: true })
      .then(({ count }) => setTotalReleases(count ?? 0));
  }, []);

  // Round 269 — Secret Messages: polls the same way NotificationBell does
  // (every 30s, matching that existing convention rather than adding a
  // second polling cadence to reason about). The nav link only renders at
  // all while secretMessages.length > 0 — same "hide the link IS the gate"
  // convention as every other role-scoped item here (see the Report entry's
  // comment above), so there is deliberately no dedicated permission check
  // beyond "does a message actually target you right now."
  //
  // The one-time popup uses localStorage (not sessionStorage) keyed by
  // profile id, so a message pops up once per person, not once per browser
  // tab/session — closing and reopening the tab a minute later shouldn't
  // re-surface something they already saw pop up once already. Only the
  // MOST RECENT unseen message pops up on any given poll (not one popup per
  // message) to avoid stacking dialogs if several arrived at once.
  //
  // Round 297 — also filters out anything this profile has hidden for
  // themselves (see secretMessages.js's getHiddenSecretMessageIds). Hiding
  // is separate from the dev-side permanent record: it only drops the
  // message out of THIS profile's badge count/link-visibility/list, same
  // "hide the link IS the gate" convention, one filter earlier.
  useEffect(() => {
    if (!supabase || !profile?.id) return;
    let cancelled = false;
    async function poll() {
      const allRows = await loadMyActiveSecretMessages(supabase, profile);
      if (cancelled) return;
      const hidden = getHiddenSecretMessageIds(profile.id);
      const rows = allRows.filter((m) => !hidden.includes(m.id));
      setSecretMessages(rows);
      let seen = [];
      try {
        seen = JSON.parse(window.localStorage.getItem(`vieent_secret_seen_${profile.id}`) || "[]");
      } catch {}
      const unseen = rows.filter((m) => !seen.includes(m.id));
      if (unseen.length > 0) {
        setSecretPopup(unseen[0]);
      }
    }
    poll();
    const interval = setInterval(poll, 30000);
    return () => { cancelled = true; clearInterval(interval); };
  }, [profile?.id]);

  function dismissSecretPopup() {
    if (!secretPopup || !profile?.id) { setSecretPopup(null); return; }
    try {
      const seen = JSON.parse(window.localStorage.getItem(`vieent_secret_seen_${profile.id}`) || "[]");
      window.localStorage.setItem(`vieent_secret_seen_${profile.id}`, JSON.stringify([...seen, secretPopup.id].slice(-200)));
    } catch {}
    setSecretPopup(null);
  }

  // Round 404 — System Messages: same poll/pop-newest-unseen pattern as
  // Secret Messages above, over the separate system_messages table. A new
  // day's row (e.g. a fresh weekly-task reminder or Bổ Sung DATA digest)
  // has a new id, so it naturally pops up once per day on its own — no
  // extra "have I seen today's version" bookkeeping needed on top of the
  // existing "seen ids" list.
  useEffect(() => {
    if (!supabase || !profile?.id) return;
    let cancelled = false;
    async function poll() {
      const allRows = await loadMyActiveSystemMessages(supabase, profile);
      if (cancelled) return;
      const hidden = getHiddenSystemMessageIds(profile.id);
      // Round 406 — also drop anything snoozed via the popup's "Hide
      // until next login" button (see hideSystemPopupUntilNextLogin
      // below) — stays out of both the badge/list and the popup pick
      // until the 24h AFK watcher further down clears the snooze list.
      const snoozed = getSnoozedSystemMessageIds(profile.id);
      const rows = allRows.filter((m) => !hidden.includes(m.id) && !snoozed.includes(m.id));
      setSystemMessages(rows);
      let seen = [];
      try {
        seen = JSON.parse(window.localStorage.getItem(`vieent_system_seen_${profile.id}`) || "[]");
      } catch {}
      const unseen = rows.filter((m) => !seen.includes(m.id));
      if (unseen.length > 0) setSystemPopup(unseen[0]);
    }
    poll();
    const interval = setInterval(poll, 30000);
    return () => { cancelled = true; clearInterval(interval); };
  }, [profile?.id]);

  function dismissSystemPopup() {
    if (!systemPopup || !profile?.id) { setSystemPopup(null); return; }
    try {
      const seen = JSON.parse(window.localStorage.getItem(`vieent_system_seen_${profile.id}`) || "[]");
      window.localStorage.setItem(`vieent_system_seen_${profile.id}`, JSON.stringify([...seen, systemPopup.id].slice(-200)));
    } catch {}
    setSystemPopup(null);
  }

  // Round 406 — the new, separate "Hide until next login" button on the
  // popup (see its render block below) — deliberately its own function
  // rather than a flag on dismissSystemPopup, since it does something
  // vanilla dismiss never does (drops the message out of the sidebar
  // badge/list too, not just "don't pop this one up again"). Marks it
  // seen the same as a normal dismiss (no point popping the same message
  // right back up on the next 30s poll) AND snoozes it — snoozeSystem
  // MessageForProfile — so it also disappears from systemMessages/the
  // badge until the 24h AFK watcher below clears it.
  function hideSystemPopupUntilNextLogin() {
    if (!systemPopup || !profile?.id) { setSystemPopup(null); return; }
    snoozeSystemMessageForProfile(profile.id, systemPopup.id);
    setSystemMessages((prev) => prev.filter((m) => m.id !== systemPopup.id));
    try {
      const seen = JSON.parse(window.localStorage.getItem(`vieent_system_seen_${profile.id}`) || "[]");
      window.localStorage.setItem(`vieent_system_seen_${profile.id}`, JSON.stringify([...seen, systemPopup.id].slice(-200)));
    } catch {}
    setSystemPopup(null);
  }

  // Round 404 items 1 & 4 — the two DAILY system-message producers.
  // Neither needs a cron: each is idempotent per (profile, day) via
  // hasSystemMessageToday/the weekly_tasks week_start check, so firing on
  // every page's Sidebar mount (this component lives in AppShell, which
  // wraps every authenticated page) is harmless — the very first mount of
  // the day for this profile actually generates something; every mount
  // after that is a cheap no-op check.
  //
  // Round 405 — mount alone isn't enough: this app has no real session
  // timeout (see Round 404's answer on that), so someone who leaves a tab
  // open across midnight never remounts Sidebar and these two never
  // re-check for a new day until they actually reload. Per explicit
  // request, "afk for more than 4 hours, count as first login on next
  // activity" — runDailyGenerators is now also wired to
  // watchForAfkReturn (lib/afkTracker.js), so a long-idle tab coming back
  // re-runs the exact same check a fresh page load would have.
  useEffect(() => {
    if (!supabase || !profile?.id) return;
    let cancelled = false;

    async function runDailyGenerators() {
      if (cancelled) return;
      // Weekly task rollover — advances any of this profile's tasks whose
      // week has moved on, then (if anything's still undone this week)
      // leaves the daily reminder for the popup effect above to pick up
      // next poll, via the same system_messages table.
      const { data: tasks } = await supabase.from("weekly_tasks").select("*").eq("assignee_profile_id", profile.id).eq("active", true);
      if (cancelled || !tasks) return;
      const { rolled, patches } = rollForwardIfNeeded(tasks);
      if (patches.length > 0) await persistRollForward(supabase, patches);
      const undone = rolled.filter((t) => !t.done);
      if (undone.length > 0) {
        const { hasSystemMessageToday, createSystemMessage } = await import("./systemMessages");
        const already = await hasSystemMessageToday(supabase, { kind: "weekly_task_reminder", profileId: profile.id });
        if (!already && !cancelled) {
          await createSystemMessage(supabase, {
            kind: "weekly_task_reminder",
            title: `${undone.length} weekly task${undone.length > 1 ? "s" : ""} still open this week`,
            body: undone.map((t) => `• ${t.text} (repeated ${t.repeat_count}x)`).join("\n"),
            link: "/task-table",
            recipientProfileIds: [profile.id],
            source: { taskIds: undone.map((t) => t.id) },
            hoursValid: 24,
          });
        }
      }

      // Bổ Sung DATA daily digest — self-gated to AR inside the function.
      if (!cancelled) ensureBoSungDataDigest(supabase, profile).catch((e) => console.error(e));
    }

    runDailyGenerators();
    const stopWatchingAfk = watchForAfkReturn(() => { runDailyGenerators(); }, DEFAULT_AFK_THRESHOLD_MS);
    // Round 406 — a second, independent watcher at the longer 24h
    // threshold, purely to reset the popup's "Hide until next login"
    // snooze (see hideSystemPopupUntilNextLogin/clearSnoozedSystemMessages
    // above) — a >24h gap is what "next login" means for that feature,
    // deliberately not the same 4h trigger the daily generators use.
    const stopWatchingSnoozeReset = watchForAfkReturn(() => {
      if (!cancelled) clearSnoozedSystemMessages(profile.id);
    }, SNOOZE_RESET_THRESHOLD_MS);
    return () => { cancelled = true; stopWatchingAfk(); stopWatchingSnoozeReset(); };
  }, [profile?.id]);

  return (
    <>
      {/* Round 269 — Secret Messages popup. Rendered fixed at the top level
          (not inside the sidebar's own fixed-width column) so it's never
          clipped by it, and above everything else (z-index 400, higher
          than TopBar's 300) since a secret message is meant to interrupt. */}
      {secretPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-strong)", borderRadius: 10, padding: 24, maxWidth: 420, boxShadow: "0 12px 40px rgba(0,0,0,0.5)" }}>
            <div style={{ fontSize: 11, color: "var(--accent)", fontWeight: 700, letterSpacing: 1, marginBottom: 10 }}>🔒 SECRET MESSAGE</div>
            <div style={{ fontSize: 13, color: "var(--text)", whiteSpace: "pre-wrap", marginBottom: 18 }}>{secretPopup.message}</div>
            <button onClick={dismissSecretPopup} style={{ background: "var(--accent)", border: "none", borderRadius: 6, padding: "8px 20px", color: "var(--accent-on)", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
              Got it
            </button>
          </div>
        </div>
      )}
      {/* Round 404 — System Messages popup. Same fixed-overlay pattern as
          the Secret Message popup above, just relabeled and wired to the
          separate systemPopup/dismissSystemPopup pair so the two
          mechanisms can never cross-contaminate each other's state. */}
      {systemPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-strong)", borderRadius: 10, padding: 24, maxWidth: 420, boxShadow: "0 12px 40px rgba(0,0,0,0.5)" }}>
            <div style={{ fontSize: 11, color: "var(--accent)", fontWeight: 700, letterSpacing: 1, marginBottom: 10 }}>🔔 SYSTEM MESSAGE</div>
            {systemPopup.title && (
              <div style={{ fontSize: 14, color: "var(--text)", fontWeight: 700, marginBottom: 8 }}>{systemPopup.title}</div>
            )}
            <div style={{ fontSize: 13, color: "var(--text)", whiteSpace: "pre-wrap", marginBottom: 18 }}>{systemPopup.body}</div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {systemPopup.link && (
                <Link
                  href={systemPopup.link}
                  onClick={dismissSystemPopup}
                  style={{ background: "transparent", border: "1px solid var(--border-strong)", borderRadius: 6, padding: "8px 20px", color: "var(--text)", textDecoration: "none", fontSize: 12, fontWeight: 700 }}
                >
                  View
                </Link>
              )}
              {/* Round 406 — "hide until the afk comeback and next afk
                  login count more than 24 hours" — a new, separate button
                  from Got it (which only stops it popping up again, but
                  leaves it in the badge/list). This one also drops it out
                  of the sidebar entirely until the 24h AFK watcher above
                  clears the snooze. */}
              <button
                onClick={hideSystemPopupUntilNextLogin}
                title="Hides this from your sidebar too — comes back once you've been away 24h+ and return"
                style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 6, padding: "8px 16px", color: "var(--text-faint)", cursor: "pointer", fontSize: 12, fontWeight: 700 }}
              >
                Hide until next login
              </button>
              <button onClick={dismissSystemPopup} style={{ background: "var(--accent)", border: "none", borderRadius: 6, padding: "8px 20px", color: "var(--accent-on)", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
                Got it
              </button>
            </div>
          </div>
        </div>
      )}
      {/* Round 87 — backdrop, mobile-drawer mode only. Tapping it closes
          the drawer the same as tapping a nav link does. */}
      {mobile && open && (
        <div
          onClick={onClose}
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 99 }}
        />
      )}
      <div
        style={{
          position: "fixed",
          top: 0,
          left: 0,
          // Round 284 — on desktop the new BottomBar starts at left:
          // SIDEBAR_WIDTH (see BottomBar.js), so it never overlaps this
          // column at all and bottom:0 here is still correct. On mobile
          // BottomBar spans the full width (left:0) and outranks this
          // drawer's zIndex 100 with its own 300, so without this the
          // drawer's bottom-most content (the theme toggle button, see
          // below) would render UNDER the fixed bottom bar whenever the
          // drawer is open — stop the column short by that same height
          // instead, same idiom AppShell already uses for page content.
          bottom: mobile ? "var(--bottombar-height)" : 0,
          width: SIDEBAR_WIDTH,
          background: "var(--bg)",
          borderRight: "1px solid var(--border)",
          display: "flex",
          flexDirection: "column",
          zIndex: 100,
          transform: open ? "translateX(0)" : "translateX(-100%)",
          transition: "transform 0.2s ease",
          boxShadow: mobile && open ? "4px 0 24px rgba(0,0,0,0.4)" : "none",
        }}
      >
      <div style={{ padding: "24px 20px 16px", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <div
              style={{
                width: 20, height: 20, borderRadius: "50%",
                background: "var(--accent)", flexShrink: 0,
              }}
            />
            <span style={{ fontWeight: 800, fontSize: 14, letterSpacing: 1 }}>VIEENT</span>
          </div>
          <div style={{ fontSize: 10, color: "var(--accent)", fontWeight: 700, letterSpacing: 1 }}>
            // PROJECT MGMT
          </div>
        </div>
        {mobile && (
          <button
            onClick={onClose}
            style={{ background: "none", border: "none", color: "var(--text-faint)", fontSize: 18, cursor: "pointer", lineHeight: 1, padding: 2 }}
          >
            ✕
          </button>
        )}
      </div>

      <div style={{ borderBottom: "2px solid var(--accent)", marginBottom: 8 }} />

      <nav style={{ flex: 1, overflowY: "auto" }}>
        {navItems.map((item) => {
          const active = pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href));
          return (
            <Link
              key={item.href}
              href={item.href}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 10,
                padding: "12px 20px",
                textDecoration: "none",
                color: active ? "var(--accent)" : "var(--text)",
                background: active ? "var(--bg-hover)" : "transparent",
                borderLeft: active ? "2px solid var(--accent)" : "2px solid transparent",
                fontSize: 13,
                fontWeight: active ? 700 : 400,
              }}
            >
              <span style={{ fontSize: 10, color: "var(--text-faint)" }}>{item.num}</span>
              {item.label}
            </Link>
          );
        })}
      </nav>

      <div style={{ borderTop: "1px solid var(--border)", padding: "10px 20px" }}>
        <Link
          href="/reference"
          style={{
            display: "block",
            padding: "6px 0",
            fontSize: 12,
            color: pathname === "/reference" ? "var(--accent)" : "var(--text-muted)",
            textDecoration: "none",
          }}
        >
          Reference
        </Link>
        <Link
          href="/config"
          style={{
            display: "block",
            padding: "6px 0",
            fontSize: 12,
            color: pathname === "/config" ? "var(--accent)" : "var(--text-muted)",
            textDecoration: "none",
          }}
        >
          Config
        </Link>
        {/* Round 269 — Secret Messages. Originally rendered ONLY while
            there's an active message targeted at this profile (unread-
            inbox-is-the-gate). Round 306 additionally renders it for
            anyone who can send (canSendSecretMessages) even with an empty
            inbox, since the Send tab now lives here too — count badge only
            shows when there's actually something unread. */}
        {(secretMessages.length > 0 || canSendSecret) && (
          <Link
            href="/secret-messages"
            style={{
              display: "flex",
              alignItems: "center",
              gap: 6,
              padding: "6px 0",
              fontSize: 12,
              color: pathname === "/secret-messages" ? "var(--accent)" : "var(--text-muted)",
              textDecoration: "none",
              fontWeight: secretMessages.length > 0 ? 700 : 400,
            }}
          >
            🔒 Secret{secretMessages.length > 0 ? ` (${secretMessages.length})` : ""}
          </Link>
        )}
        {/* Round 404 item 3 — System Messages. Same "unread inbox is the
            gate" convention as Secret Messages above, but with no
            canSendSecret-style always-on branch since nobody composes
            these by hand — the only reason to show the link outside of
            "you have one" is dev's separate log view just below. */}
        {systemMessages.length > 0 && (
          <Link
            href="/system-messages"
            style={{
              display: "flex",
              alignItems: "center",
              gap: 6,
              padding: "6px 0",
              fontSize: 12,
              color: pathname === "/system-messages" ? "var(--accent)" : "var(--text-muted)",
              textDecoration: "none",
              fontWeight: 700,
            }}
          >
            🔔 System ({systemMessages.length})
          </Link>
        )}
        {/* Round 404 item 3 — "Dev still see it in the log": every
            system_messages row ever, ignoring expires_at, dev-only same
            gate convention as everything else in this file (no real RLS —
            see the Report nav item's comment above). */}
        {isDev && (
          <Link
            href="/system-log"
            style={{
              display: "block",
              padding: "6px 0",
              fontSize: 12,
              color: pathname === "/system-log" ? "var(--accent)" : "var(--text-muted)",
              textDecoration: "none",
            }}
          >
            System Log
          </Link>
        )}
        {/* Round 58 — Package Runner. Only shown to admins on the
            Marketing team + dev (see canRunPackageSimulator), unlike
            Config/Reference above which everyone sees (Config self-gates
            per-tab instead). This one's a niche operational tool, not
            worth surfacing to roles that can never use it. */}
        {showPackageRunner && (
          <Link
            href="/package-runner"
            style={{
              display: "block",
              padding: "6px 0",
              fontSize: 12,
              color: pathname === "/package-runner" ? "var(--accent)" : "var(--text-muted)",
              textDecoration: "none",
            }}
          >
            Package Runner
          </Link>
        )}
      </div>

      <div style={{ borderTop: "1px solid var(--border)", padding: "16px 20px" }}>
        <div style={{ fontSize: 10, color: "var(--accent)", fontWeight: 700, letterSpacing: 1, marginBottom: 6 }}>
          // TOTAL
        </div>
        <div style={{ fontSize: 26, fontWeight: 800, marginBottom: 2 }}>
          {totalReleases === null ? "…" : totalReleases}
        </div>
        <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>Releases</div>

        <button
          onClick={toggleTheme}
          style={{
            width: "100%",
            background: "transparent",
            border: "1px solid var(--border-strong)",
            borderRadius: 6,
            padding: "8px 0",
            fontSize: 11,
            fontWeight: 700,
            color: "var(--text)",
            cursor: "pointer",
          }}
        >
          {theme === "dark" ? "☀ LIGHT" : "☾ DARK"}
        </button>
      </div>
      </div>
    </>
  );
}
