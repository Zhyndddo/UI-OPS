...
// Same real-assignable-team list as app/config/page.js's TEAMS — Round
// 262 folded Youtube/Publishing/Operation back into "OPS" (see
// lib/teamTypes.js's header comment).
const VIEW_AS_TEAMS = ["AR", "Marketing", "Design", "OPS", "Legal"];
...
export default function TopBar({ isMobile, onMenuClick, showReleaseSearch, toolsPageKey }) {
  ...
  // Round 246 — fixed instead of sticky, per explicit request. Sticky only
  // stays pinned within its own scroll context; it silently breaks the
  // moment any ancestor between here and the scrolling document gets an
  // overflow/transform/filter of its own (a real risk — the app has more
  // than a few of those). Fixed is anchored to the viewport unconditionally,
  // so it can never come unstuck that way. Trade-off: fixed is taken
  // completely out of flow, so AppShell.js now has to reserve the space
  // itself (paddingTop: var(--topbar-height) on the content column) instead
  // of that space existing automatically — see the comment there. left is
  // pinned to the sidebar's right edge (0 on mobile, where the sidebar is
  // an off-canvas drawer instead of a docked column) so the bar spans only
  // the content column, matching what sticky looked like before.
  return (
    <div style={{ position: "fixed", top: 0, left: isMobile ? 0 : SIDEBAR_WIDTH, right: 0, zIndex: 300 }}>
      <div
      ...
      >
        ...
        {/* Round 254 — per explicit report: clicking near the bell or the
            account name often "missed" and fell through to the bar's own
            onClick above (navigate to Dashboard), because both were only
            ever as clickable as their own tight icon/text bounds. Two
            layers of fix, both invisible (no border, no visible change):
            this whole cluster now also stops propagation on its own — so
            even a click in the GAP between the bell and the name (still
            technically inside the orange bar) never reaches the
            navigate-to-dashboard handler — and each of the two triggers
            below additionally gets real extra padding offset by an equal
            negative margin, which grows its actual clickable hit box
            outward into the surrounding whitespace without changing
            anything about the bar's visual layout or spacing. */}
        <div style={{ display: "flex", alignItems: "center", gap: 14 }} onClick={(e) => e.stopPropagation()}>
          ...
          <div
            onClick={() => setOpen((o) => !o)}
            style={{ cursor: "pointer", padding: "10px 8px", margin: "-10px -8px" }}
          >
