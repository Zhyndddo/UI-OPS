...
// Round 396 — "splitshare label field now searchable (by typing in)".
// Same free-text-with-suggestions idiom as LabelInput/ArtistInput above
// (this file's whole reason to exist), but over a flat list of strings
// instead of a {stage_name}/{label_name} row — Split Share's "Shared
// Label" (lib/GateFields.js's split_share_entries editor) was never a
// foreign key into the labels/artists tables, it's freeform text
// recording whichever outside label/entity a release's rights are being
// split with, so there's no reference table to query here — `options` is
// whatever distinct shared_label values already exist across every
// release's split_share_entries (see app/releases/[id]/page.js's
// OverviewTab, where that list gets fetched once and passed down through
// GateFields).
export function SharedLabelInput({ value, onChange, options, placeholder, styles }) {
  const [open, setOpen] = useState(false);
  const matches =
    (value || "").trim().length > 0
      ? (options || []).filter((o) => o.toLowerCase().includes(value.trim().toLowerCase())).slice(0, 8)
      : (options || []).slice(0, 8); // empty field — show a starter list instead of nothing, same "browse or type" affordance a <select> would give
