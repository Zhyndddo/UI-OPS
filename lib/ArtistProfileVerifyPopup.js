        ...
        <div style={{ flex: 1, minWidth: 0, padding: 20, overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div>
              ...
              {/* Round 419 — "add the label for these section": which
                  request types live under this card, so it's clear at a
                  glance without opening every row below. */}
              <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 2 }}>
                {cardOptionLabels(activeCard).length} option{cardOptionLabels(activeCard).length === 1 ? "" : "s"}: {cardOptionLabels(activeCard).join("; ")}
              </div>
            </div>
