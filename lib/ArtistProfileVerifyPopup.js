...
// Round 419 — per explicit request ("add the label for these section"):
// each card's option list, one source of truth (REQUEST_TYPES /
// ARTIST_VERIFY_CARD_TYPES) instead of a hand-typed string that could
// drift from the real request types. Verification's 2 options aren't a
// REQUEST_TYPES "group" (its two keys — verification/new — both carry
// group: "new", which means something different: "this is a NEW-artist
// flow, not a management one"), so it reads straight off
// ARTIST_VERIFY_CARD_TYPES instead; Management's 5 options are the real
// group: "management" entries.
function cardOptionLabels(cardKey) {
  if (cardKey === "verification") return ARTIST_VERIFY_CARD_TYPES.map(requestTypeLabel);
  return REQUEST_TYPES.filter((t) => t.group === "management").map((t) => t.label);
}
        ...
        <div style={{ width: 40, flexShrink: 0, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 16, gap: 8, background: "var(--bg)" }}>
          {CARDS.filter((c) => c.key !== activeCard).map((c) => (
            <button
              ...
              title={`${c.label} — ${cardOptionLabels(c.key).join(", ")}`}
              style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {ARTIST_VERIFY_CARD_TYPES.map((type) => {
        // Round 419 follow-up — per explicit correction ("i mean to add
        // right on the collaspable as a label"): the label belongs ON the
        // Artist Verification/NEW Profile bar itself, not up in the card
        // header. Same eligible-count ArtistTypeList already computes
        // below (artist tags on this release that don't already have a
        // ticket of this exact type) — shown as a small badge right next
        // to the title, so it's clear at a glance whether there's
        // anything to review before opening it.
        const eligibleCount = artistTags.filter((name) => !existingKeys.has(artistVerifyKey(name, type))).length;
        return (
          <CollapsibleSection key={type} title={requestTypeLabel(type)} count={eligibleCount}>
...
// `count` (Round 419 follow-up) is an optional small badge next to the
// title — currently only Artist Verification's two rows pass one.
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        style={{ width: "100%", textAlign: "left", background: "var(--bg)", color: "var(--text)", border: "none", padding: "10px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
      >
        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {count != null && (
            <span
              style={{
                fontSize: 10, fontWeight: 700, color: count > 0 ? "#ff6b1a" : "var(--text-faint)",
                background: count > 0 ? "rgba(255,107,26,0.14)" : "var(--bg-hover)",
