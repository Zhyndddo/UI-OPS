...
export default function PhuLucStyleTicketList({ typeKey, basePath, title, differentiator, urlLabel, hideGiaTri }) {
  ...
  useEffect(() => {
    if (!supabase) return;
    load();
    // Round 78 — PIC list filtered to Legal (the executor team for both
    // Phụ Lục MG and Phụ Lục Publishing), dev excluded — see filterProfilesByTeam.
    supabase.from("profiles").select("id, name, segment, role").order("name").then(({ data }) => setProfiles(filterProfilesByTeam(data || [], "Legal")));
  }, []);
  ...
  async function updatePic(t, profileId) {
    ...
    logPicReassign({ actor: profile?.id, entity: "ticket", entityId: t.id, before: t.pic_profile_id, after: patch.pic_profile_id });
  }
  ...
  async function updateStatus(t, newStatus) {
    ...
    // into ticket.data.note (see lib/statusNoteGate.js). This list has no
    // Note column of its own, so the reason surfaces via a hover on the
    // Status cell instead (see the <td title=...> below).
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                <th>Ngày Order</th>
                <th>Release</th>
                {/* Round 81 item 5 — hidden for Phụ Lục Publishing
                    specifically (hideGiaTri), per explicit request; Phụ
                    Lục MG still gets this column as before. */}
                {!hideGiaTri && <th>{`Giá Trị PL (${differentiator})`}</th>}
