...
// handler (app/[slug]/route.js). See sql/pending/add-round360-short-
// links.sql for the table this reads/writes.

// Every existing top-level route this app already serves at "/<name>" —
// a slug matching one of these could never actually be reached (Next.js
// always resolves a real static route before falling through to the
// app/[slug] catch-all), so minting one would create a link that looks
// like it works but silently 404s at the real page instead of
// redirecting. Kept as a plain list (not derived from the filesystem —
// Route Handlers can't read the project's own folder structure at
// runtime in a deployed/serverless build) mirroring the current
// top-level app/ folders; add to this list if a new top-level route is
// ever added.
...
// proxied — it's a page this app renders directly — so app/[slug]/
// route.js can REWRITE to it (browser keeps showing /vsounder, no
// redirect) instead of bouncing the visitor away. A destination on any
// other hostname still gets a normal redirect — see that route's own
