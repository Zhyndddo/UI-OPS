...
// Round 86 follow-up item 6 — when either timing pick is the "same day"
// option ("Cùng Ngày"/"Cùng ngày" — Tiktok and Facebook spell it with
// different capitalization, see LINKSHARE_TIKTOK_OPTIONS/
// LINKSHARE_FACEBOOK_OPTIONS below), the note now appends the release
// date right after it (e.g. "Cùng ngày 20/08/2026") instead of leaving it
// as a bare "same day" with no actual date attached. The other options
// ("+4"/"+7") already spell out their own offset in words, so they're
// left as-is.
//
...
function timingWithDate(timing, releaseDate) {
  ...
  if (!SAME_DAY_TIMINGS.includes(timing) || !datePart) return timing;
  return `${timing} ${datePart}`;
}
