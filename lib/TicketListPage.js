...
const COMPUTED_REPLACES = {
  ...
  // Round 325 — Khác's new "selfTask" checkbox (lib/ticketConfigs.js) is a
  // visibility flag, not a displayable data field — reusing this
  // "excluded from the list view" mechanism keeps it out of previewFields
  // (and off the generic plain-text-input renderer, which would otherwise
  // show/edit a raw true/false) without a new field-hiding concept. It's
  // shown instead as a small badge — see the "🔒 Personal" marker below.
  khac: ["selfTask"],
};
...
// Round 325 — "Self Task" (lib/ticketConfigs.js's khac.selfTask field):
// per explicit request, a Khác ticket someone creates for themselves only
// ("personal note") should only ever be visible to its own requester, not
// to every other viewer of the (normally fully shared/"Tất cả") Khác tab.
// Applied to every ticket-fetch query below via PostgREST .or(), not just
// khac's — data->>selfTask is null for every OTHER ticket type (the field
// doesn't exist on them), so this is always a no-op filter for them,
// matching everything as before. Same reasoning as the visibility check:
// a row is visible if it's not a self task at all, OR it is one and the
// viewer is its own requester.
function selfTaskVisibilityFilter(profileId) {
  const mine = profileId ? `,requester_profile_id.eq.${profileId}` : "";
  return `data->>selfTask.is.null,data->>selfTask.eq.false${mine}`;
}
...
export default function TicketListPage({ typeKey, basePath, externalLink }) {
  ...
  async function fetchBuffer(tabRow, status, anchorPage, size) {
    ...
    const { data } = await supabase
      ...
      .is("deleted_at", null)
      .eq("status", status)
      .or(selfTaskVisibilityFilter(profile?.id))
...
function TicketRow({ ticket, index, previewFields, computedCols, config, tab, profiles, isExecutorView, onUpdateField, onUpdateStatus, onUpdatePic, onAcknowledgeEdit, mobile = false, releaseOverride = null, release = null }) {
  ...
  // Round 325 — visible marker for a Self Task, since it's excluded from
  // previewFields (see COMPUTED_REPLACES) so it doesn't render as a raw
  // true/false data field. Only ever seen by its own requester anyway
  // (the fetch-side visibility filter hides it from everyone else), but
  // it's still worth flagging so that person can tell it apart from a
  // real "Khác" request at a glance.
  const selfTaskBadge = ticket.data?.selfTask ? (
    <span title="Only visible to you" style={{ fontSize: 10, fontWeight: 700, color: "var(--text-faint)", border: "1px solid var(--border)", borderRadius: 4, padding: "2px 6px", whiteSpace: "nowrap" }}>
      🔒 Personal
    </span>
  ) : null;
    ...
    return (
      <div style={{ border: "1px solid var(--border)", borderRadius: 10, padding: 14, background: showEditedHighlight ? "rgba(255,107,26,0.06)" : "var(--bg-card)", boxShadow: showEditedHighlight ? "inset 3px 0 0 var(--accent)" : undefined }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }} title={ticket.data?.note || undefined}>
            {selfTaskBadge}
            {statusBody}
          </div>
        </div>
