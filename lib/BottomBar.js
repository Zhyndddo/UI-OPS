...
// image). No more hand-built HTML text lockup. That white text is
// invisible on the light theme's light page background (confirmed by
// compositing the image onto white before trusting it), so per explicit
// request ("ON lighter theme... make a black rounded square surrounding
// it") the light theme ONLY wraps the image in a fixed black rounded
// square. Dark and Cosmic already have a dark page background behind the
// bottom bar, so the image sits directly on it there with no wrapper —
// unlike Round 317's pill, this one genuinely needs to follow the theme
// (useTheme), not stay fixed across all of them.
function EmpoweredByBadge({ compact, theme }) {
  const imgHeight = compact ? 22 : 32;
  const badge = (
    <img
      src="/brand/vieent-badge.png"
