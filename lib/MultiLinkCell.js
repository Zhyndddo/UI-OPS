          ...
          {lines.map((line, i) =>
            URL_RE.test(line) ? (
              <a
                ...
                style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--accent)", fontSize: 12 }}
              >
                {line}
              </a>
            ) : (
              <span key={i} style={{ fontSize: 12, color: "var(--text-faint)", whiteSpace: "pre-wrap" }}>
                {line}
              </span>
            )
          )}
