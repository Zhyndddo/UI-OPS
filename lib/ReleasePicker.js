...
export default function ReleasePicker({ onSelect, excludeDids }) {
  ...
  useEffect(() => {
    ...
    // Round 144 — isrc/upc added so Report Conflict's expanded auto-fill
    // (Official ISRC/Official UPC, see app/tickets/report-conflict/new/
    // page.js) can read them straight off the picked release without a
    // second query. Purely additive — every other caller just ignores the
    // extra columns.
    supabase
      .from("releases")
      .select("id, did, title, main_artist, label, release_date, isrc, upc")
