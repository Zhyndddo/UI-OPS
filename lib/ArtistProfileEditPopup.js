...
import { requestTypeLabel, fieldsForType, platformOptionsForType, ALL_PLATFORMS, isLegacyTicket, mergeNamesMismatch } from "./artistProfileRequestTypes";
...
export default function ArtistProfileEditPopup({ styles, ticket, tab, profiles, isExecutorView, onUpdateData, onUpdatePic, onUpdateStatus, onClose }) {
  ...
  // Round 414 — Verification/NEW Profile tickets created from the release
  // detail page's popup hold data.platforms (multi-select array); every
  // other type created via the OLD manual "+ New Ticket" popup (still
  // untouched — see lib/NewArtistProfileTicketPopup.js) uses the older
  // singular data.platform. Round 415 (Phase 2) — Profile Management
  // tickets created from the release detail page's popup ALSO hold
  // data.platforms now (see lib/ArtistProfileVerifyPopup.js's
  // ProfileManagementCard), so this check widens to "does this ticket
  // actually carry an array" rather than a fixed type list — it covers
  // both entry points' multi-platform shape without caring which type
  // created it. Legacy tickets (pre-166) keep the singular select too —
  // they never had a multi-select concept.
  const isMultiPlatformTicket = !legacy && Array.isArray(ticket.data?.platforms);
  const isManagementType = !legacy && ["update_bio", "add_song", "remove_song", "merge", "transfer"].includes(requestType);
  const color = statusColor(ticket.status);
  // Old single-field rendering path (legacy tickets, and any non-multi-
  // platform ticket from the manual popup) still filters out the identity
  // fields it renders separately above/below — unchanged from Round 414.
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            ...
            {isMultiPlatformTicket ? (
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {platformOptions.map(([k, label]) => {
                  const current = ticket.data?.platforms || [];
                  return (
                    <label key={k} className={styles.checkboxRow} style={{ fontSize: 12 }}>
                      <input
                        type="checkbox"
                        checked={current.includes(k)}
                        onChange={(e) => onUpdateData({ platforms: e.target.checked ? [...new Set([...current, k])] : current.filter((x) => x !== k) })}
                      />
                      {label}
                    </label>
                  );
          ...
          {!legacy && requestType === "transfer" ? (
            <>
              {/* Round 415 — for a multi-platform (new-flow) transfer
                  ticket, oldStageName is now a perPlatform field (spec:
                  "add field for each platform ticked") — rendered below by
                  ArtistProfileFieldGroup, not here. A legacy-shape/manual-
                  popup transfer ticket still has oldStageName as a single
                  scalar, so it keeps the plain input. */}
              {!isMultiPlatformTicket && (
                <div>
          ...
          ) : isManagementType && isMultiPlatformTicket ? (
            // Round 415 — a Profile Management ticket created from the
            // release detail page's new flow: reuse the same per-platform
            // field renderer that popup uses, passing onUpdateData straight
            // through as onChange (same (key, fullNewValue) signature).
            <ArtistProfileFieldGroup
              styles={styles}
              type={requestType}
              platforms={ticket.data?.platforms || []}
              values={ticket.data}
              onChange={(key, value) => onUpdateData({ [key]: value })}
            />
          ) : (
            fields.map((f) => {
              if (f.key === "latestSong") {
          ...
          {/* Round 196 — same non-blocking warning as the creation popup;
              reads straight off the saved ticket data (fields above write
              onBlur, so this reflects the value as of the last blur, same
              as every other field here). */}
          {!legacy && requestType === "merge" && mergeNamesMismatch(ticket.data?.wrongProfileName, ticket.data?.correctProfileName) && (
            <div style={{ fontSize: 11, color: "#ffca4d", background: "rgba(255,202,77,0.1)", border: "1px solid #5a4a1a", borderRadius: 6, padding: "8px 10px" }}>
              ⚠ Tên hiển thị trên 2 profile không trùng nhau — kiểm tra lại trước khi gộp.
            </div>
          )}
