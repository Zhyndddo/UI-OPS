...
const DUAL_VIEW_EXECUTOR_TEAM = {
  phai_sinh: "OPS",
  // Batch Phái Sinh's dual view is on the PARENT ticket (create/view the
  // batch) — the same OPS-executes/AR-requests split as plain Phái Sinh.
  // Its "not done" COUNT, though, is bespoke (see ticketNotDoneCount
  // below) — per explicit request, each song counts as its own workload
  // item, not the batch as one.
  batch_phai_sinh: "OPS",
  manual_claim: "OPS",
  report_conflict: "OPS",
  artist_profile: "OPS",
...
async function ticketNotDoneCount(typeKey, profile) {
  // Bespoke: count children (phai_sinh_batch_items rows), not the parent
  // batch tickets — "treat each children row a workload row aka N item
  // rather 1 item per batch" per explicit request. The requester/executor
  // split doesn't change this type's items (everyone sees the same set of
  // songs), only whether COMPLETE/CANCELED count as terminal, same
  // asymmetric REFUND-esque rule as everything else — but this type has
  // no REFUND-equivalent state, so COMPLETE/CANCELED are terminal for
  // both sides.
  if (typeKey === "batch_phai_sinh") {
    const { data: items } = await supabase.from("phai_sinh_batch_items").select("status").is("deleted_at", null);
    if (!items) return 0;
    return items.filter((i) => !["COMPLETE", "CANCELED"].includes(i.status)).length;
  }
