...
// Round 401 — daily digest's "Ticket Counter — Not Done" / "Workstation —
// Not Done" sections wanted to stop counting genuinely old backlog rows
// ("exclude anything (created or release date) before august of 2026").
// Threaded through as an OPTIONAL param, default undefined/null — every
// existing caller (TypeSwitcher's tab badges, app/workstation/page.js,
// app/tickets/page.js) keeps counting everything, unfiltered, exactly as
// before. Only the digest route (app/api/cron/daily-digest/route.js)
// passes a real sinceDate, scoped to just those two sections.
function scopeCreatedAt(query, sinceDate) {
  return sinceDate ? query.gte("created_at", sinceDate) : query;
}

// Same idea for the release-backed workstation counts, but keyed off
// release_date instead of created_at (these are "how much release work is
// still outstanding," not "how recently was this row created") — and a
// release with no release_date set yet is deliberately kept rather than
// excluded: an undated release isn't "before August," it's just not
// scheduled yet, and it's exactly the kind of thing this list should still
// surface as outstanding work.
function scopeReleaseDate(query, sinceDate) {
  return sinceDate ? query.or(`release_date.is.null,release_date.gte.${sinceDate}`) : query;
}

async function ticketNotDoneCount(typeKey, profile, sinceDate) {
  ...
  if (typeKey === "phai_sinh") {
    ...
    if (!tab) return null;
    const { data: tickets } = await scopeCreatedAt(supabase.from("tickets").select("id, status, data").eq("tab_id", tab.id).is("deleted_at", null), sinceDate);
    if (!tickets) return 0;
  ...
  // Round 428 — "i think it may be due to the long old tickets in the
  // request status that belong to old release still counting": Media
  // Booking's not-done count was only ever scoped by the TICKET's own
  // created_at (scopeCreatedAt below, same as every other generic type).
  // That's the wrong old-ness signal here — a Media Booking ticket can
  // sit open/REQUESTED indefinitely against a release from long ago
  // without its own created_at ever being touched again, or (less
  // obviously) get a fresh created_at from something unrelated to the
  // release's actual age. The real "is this old backlog" question for
  // this type is the underlying RELEASE's release_date, same idea as
  // workstationNotDoneCount's scopeReleaseDate branches use for
  // release-backed workstation rows. Tickets link to their release via
  // data.releaseId -> releases.did (see app/tickets/media-booking/
  // page.js's own RELEASE_COLUMNS_FOR_TICKETS/dids pattern — PostgREST
  // can't join ticket.data->>'releaseId' directly, so this is two
  // fetches + a JS-side map, same as that page does). A ticket whose
  // release can't be resolved (no releaseId, release since deleted) or
  // whose release has no release_date set yet is deliberately KEPT, not
  // excluded — same "don't drop what we can't verify / undated isn't
  // stale, it's just unscheduled" rule scopeReleaseDate documents above.
  if (typeKey === "media_booking") {
    const { data: tab } = await supabase.from("ticket_tabs").select("id").eq("key", "media_booking").single();
    if (!tab) return null;
    const { data: tickets } = await scopeCreatedAt(supabase.from("tickets").select("status, data").eq("tab_id", tab.id).is("deleted_at", null), sinceDate);
    if (!tickets) return 0;
    const executor = isExecutorView("media_booking", profile);
    const terminal = executor ? TERMINAL_EXECUTOR : TERMINAL_REQUESTER;
    const notDone = tickets.filter((t) => !terminal.includes(t.status));
    if (!sinceDate) return notDone.length;
    const dids = [...new Set(notDone.map((t) => t.data?.releaseId).filter(Boolean))];
    let releaseDateByDid = {};
    if (dids.length > 0) {
      const { data: rels } = await supabase.from("releases").select("did, release_date").in("did", dids);
      (rels || []).forEach((r) => { releaseDateByDid[r.did] = r.release_date; });
    }
    return notDone.filter((t) => {
      const releaseDate = releaseDateByDid[t.data?.releaseId];
      return !(releaseDate && releaseDate < sinceDate);
    }).length;
  }

  const { data: tab } = await supabase.from("ticket_tabs").select("id").eq("key", typeKey).single();
  if (!tab) return null;
  const { data: tickets } = await scopeCreatedAt(supabase.from("tickets").select("status").eq("tab_id", tab.id).is("deleted_at", null), sinceDate);
  if (!tickets) return 0;
...
async function workstationNotDoneCount(typeKey, sinceDate) {
  if (typeKey === "upload") {
    const { data } = await scopeReleaseDate(
      supabase.from("releases").select("upload_status, link_lbm, link_share, smartlink, link_preorder, gate_pre_order").eq("requested", true),
      sinceDate
    );
    if (!data) return 0;
  ...
  if (typeKey === "pitching") {
    ...
    const { data: rels } = await scopeReleaseDate(
      supabase.from("releases").select("did, upc, priority_pitching, pitching_status_spotify, pitching_status_apple, pitching_status_nct, pitching_status_zing").in("did", dids),
      sinceDate
    );
    const releaseMap = {};
  ...
  if (typeKey === "pre_release") {
    ...
    // there for now.
    const { data } = await fetchAllRows(() =>
      scopeReleaseDate(supabase.from("releases").select("canva_mv_status, canva_status, musixmatch_link, musixmatch_status, nct_lyric, zing_lyric").order("id"), sinceDate)
    );
    if (!data) return 0;
...
function notDoneCountCacheKey(kind, typeKey, profile, sinceDate) {
  // Round 401 — sinceDate folded into the cache key so the digest route's
  // date-scoped calls (see scopeCreatedAt/scopeReleaseDate above) never
  // collide with, or get served, every other caller's unfiltered count —
  // each distinct sinceDate (including "none") gets its own cache entry.
  return `${kind}:${typeKey}:${profile?.id || "anon"}:${sinceDate || "all"}`;
}
...
//
// opts.sinceDate (optional, "YYYY-MM-DD") — Round 401: excludes rows older
// than this date (created_at for tickets, release_date for releases — see
// scopeCreatedAt/scopeReleaseDate above). Every existing caller omits this
// and keeps counting everything unfiltered; only the daily digest route
// passes it, scoped to just its two "— Not Done" sections.
export async function getNotDoneCount(kind, typeKey, profile, opts) {
  const sinceDate = opts?.sinceDate || null;
  const key = notDoneCountCacheKey(kind, typeKey, profile, sinceDate);
  ...
  const promise = (async () => {
    try {
      if (kind === "ticket") return await ticketNotDoneCount(typeKey, profile, sinceDate);
      return await workstationNotDoneCount(typeKey, sinceDate);
    } catch {
      return null;
    }
  })();
