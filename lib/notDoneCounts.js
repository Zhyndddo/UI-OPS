...
async function workstationNotDoneCount(typeKey) {
  ...
  if (typeKey === "confirm") {
    ...
    const { data } = await fetchAllRows(() =>
      supabase.from("releases").select([...DSP_CHECK_FIELDS, "link_lbm", "smartlink", "confirm_insta_sound", "confirm_tiktok_sound_updated", "confirm_smartlink_updated"].join(", ")).order("id")
    );
    if (!data) return 0;
  ...
  if (typeKey === "pre_release") {
    ...
    const { data } = await fetchAllRows(() =>
      supabase.from("releases").select("canva_mv_status, canva_status, artist_pick_status, musixmatch_link, musixmatch_status, nct_lyric, zing_lyric").order("id")
    );
    if (!data) return 0;
