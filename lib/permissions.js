...
// Round 258 — the "INDIE" project flag (see sql/pending/
// add-round258-indie-flag.sql): per explicit request, "team lead role,
// marketing team" specifically — not admin/exc on Marketing, not team
// leads on other teams. dev keeps the same global-bypass exception every
// other capability check in this file already gives it.
export function canFlagIndie(profile) {
  return isDev(profile) || (profile?.role === "teamlead" && profile?.segment === "Marketing");
}
...
export function canViewSubteamColumn(profile, subteamName) {
  return isDev(profile) || (profile?.role === "teamlead" && !!profile?.subteam && profile.subteam === subteamName);
}
...
// dev gets every subteam that exists anywhere (allSubteams, the full
// distinct list); a team lead gets just their own (a 1-item list, or
// empty if they have no subteam set yet); everyone else gets none.
export function visibleSubteamsFor(profile, allSubteams) {
  if (isDev(profile)) return allSubteams;
  if (profile?.role === "teamlead" && profile?.subteam) return [profile.subteam];
  return [];
}
