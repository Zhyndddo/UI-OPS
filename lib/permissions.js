...
// Round 261 — subteam tagging. Per explicit spec: "each subteam has
// their own is_subteam tag for a product that only they can see... only
// the team leader" can view/change it — deliberately NOT admin, "for
// now" ("the more subteam the more complicated and crowded the admin
// view"). dev keeps the usual global-bypass exception every other check
// in this file already gives it (here: sees/edits every subteam's
// column, not just one — accepted since dev already gets everything
// else too).
//
...
export function canViewSubteamColumn(profile, subteamName) {
  return isDev(profile) || (profile?.role === "teamlead" && profile?.segment === SUBTEAM_TAG_TEAM && !!profile?.subteam && profile.subteam === subteamName);
}
...
// already be pre-filtered to Marketing by the caller); a Marketing team
// lead gets just their own (a 1-item list, or empty if they have no
// subteam set yet); everyone else — including a team lead on any other
// team — gets none.
export function visibleSubteamsFor(profile, allSubteams) {
  if (isDev(profile)) return allSubteams;
  if (profile?.role === "teamlead" && profile?.segment === SUBTEAM_TAG_TEAM && profile?.subteam) return [profile.subteam];
  return [];
}
