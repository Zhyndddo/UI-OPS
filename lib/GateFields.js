...
// Small "Send Ticket"/"Open Ticket" affordance shown once a mapped gate
// field is "Yes" — mirrors the Pitching field's existing "Send Ticket to
// Collect Info" button idiom. ticketMap is keyed by ticket type key (not
// gate field key) -> the existing ticket row for this release, or
// undefined if none has been sent yet. Both ticketMap and onSendTicket are
// optional — the New Release create form (no release/DID to attach a
// ticket to yet) passes neither, so this renders nothing there.
function GateTicketLink({ styles, gateKey, ticketMap, onSendTicket }) {
  const ticketType = GATE_TICKET_TYPES[gateKey];
  if (!ticketType || !onSendTicket) return null;
  ...
  return (
    <button type="button" className={styles.btnSmall} style={{ marginTop: 6 }} onClick={() => onSendTicket(ticketType)}>
      Send Ticket
    </button>
  );
}
...
export function GateGrid({ styles, fields, form, update, suppressUrlFor, ticketMap, onSendTicket }) {
  return (
      ...
      {fields.map(([key, label, opts]) => {
        ...
        return (
          <div key={key} className={styles.field} style={{ marginBottom: 0 }}>
            ...
            {form[key] === "true" && (
              <GateTicketLink styles={styles} gateKey={key} ticketMap={ticketMap} onSendTicket={onSendTicket} />
            )}
          </div>
        );
      })}
...
export function GateFields({ styles, form, update, pitchingTypes, onPitchingToggle, suppressUrlFor, pitchingInfoTicket, onSendPitchingInfoTicket, ticketMap, onSendTicket }) {
  ...
  return (
    <div>
      ...
      <GateGrid styles={styles} fields={DATA_REQUEST_FIELDS} form={form} update={update} suppressUrlFor={suppressUrlFor} ticketMap={ticketMap} onSendTicket={onSendTicket} />
      ...
      <GateGrid styles={styles} fields={MARKETING_REQUEST_FIELDS} form={form} update={update} suppressUrlFor={suppressUrlFor} ticketMap={ticketMap} onSendTicket={onSendTicket} />

      <div className={styles.subheading}>Legal Request</div>
      <GateGrid styles={styles} fields={LEGAL_REQUEST_FIELDS} form={form} update={update} suppressUrlFor={suppressUrlFor} ticketMap={ticketMap} onSendTicket={onSendTicket} />
