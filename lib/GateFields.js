...
export function GateFields({ styles, form, update, pitchingTypes, onPitchingToggle, onArtistProfileYes }) {
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 12 }}>
        {GATE_FIELDS.map(([key, label]) => {
          const urlField = URL_GATE_FIELDS[key];
          return (
            <div key={key} className={styles.field} style={{ marginBottom: 0 }}>
              <label className={styles.fieldLabel} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {label}
                {urlField && form[urlField] && (
                  <a href={form[urlField]} target="_blank" rel="noopener noreferrer" title="Open link" style={{ fontSize: 12 }}>
                    🔗
                  </a>
                )}
              </label>
              <GateToggle
                ...
                onChange={(v) => {
                  update(key, v);
                  if (key === "gate_artist_profile" && v === "true") onArtistProfileYes?.();
                }}
              />
