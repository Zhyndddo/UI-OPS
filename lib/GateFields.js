...
import { SharedLabelInput } from "./ReferenceInputs";
import ArtistProfileVerifyPopup from "./ArtistProfileVerifyPopup";
import { ARTIST_VERIFY_CARD_TYPES } from "./artistProfileRequestTypes";
...
// Round 97 — Artist Profile Verify's own panel; Round 172 collapsed the
// always-inline checklist into a popup. Round 414 — the popup itself moved
// out to its own file, lib/ArtistProfileVerifyPopup.js (see that file's
// header for the full rework — per-artist mini-forms instead of one flat
// checklist + one shared platform draft, real per-request-type tickets
// instead of always the legacy shape, and the new card/tab shell for
// Profile Management). Imported above; only the trigger card + state
// live here now.
...
// Round 217 — `lockYes` (optional): once true AND the toggle is
// currently "true" (Yes), every OTHER option becomes unclickable — No
// and TBU still swap freely with each other, but Yes can never be
// walked back off of once it's locked. Callers decide when that
// condition holds (see the Metadata Checklist in
// app/releases/[id]/page.js — locks a required field once Send Upload
// has gone out AND that field's last-SAVED value is genuinely "Yes").
// A field that's still No/TBU post-send stays fully open — the lock
// only ever engages on a field that's already Yes.
export function GateToggle({ value, onChange, lockYes, lockTitle }) {
  const isLocked = lockYes && value === "true";
  return (
    <div style={{ display: "flex", border: "1px solid var(--border-strong)", borderRadius: 6, overflow: "hidden" }}>
      {GATE_OPTIONS.map((o) => {
        const disabled = isLocked && o !== "true";
        return (
          <button
            key={o}
            type="button"
...
// Round 290 — label renamed Giá Trị Publishing -> Tỉ Lệ Sở Hữu, per
// explicit request (display text only — the underlying field name,
// publishing_gia_tri, is unchanged). Also added extraField/extraLabel:
// Composer, free text for now, collected in the same popup — see
// GateGrid's openTextField rendering below, the one place that reads
// this shape.
const TEXT_GATE_FIELDS = {
  gate_publishing: { field: "publishing_gia_tri", label: "Tỉ Lệ Sở Hữu", extraField: "publishing_composer", extraLabel: "Composer" },
};
...
function GateTicketLink({ styles, gateKey, ticketMap, sonyPublishMetaReady }) {
  ...
  if (gateKey === "gate_publishing") {
    return (
      <p style={{ color: "var(--warn-fg)", fontSize: 11, marginTop: 6, marginBottom: 0 }}>
        ⚠ Fill in Tỉ Lệ Sở Hữu above, then Save to send.
      </p>
    );
  }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 12 }}>
      ...
      {openTextField && (
        <GatePopupShell title={openLabel} onClose={() => setOpenField(null)} width={440}>
          ...
          />
          {/* Round 290 — Composer, free text for now, per explicit
              request: "when tick yes... also do a popup new fields, label
              composer." Same popup as the field above, not a separate one. */}
          {openTextField.extraField && (
            <input
              className={styles.input}
              style={{ width: "100%", boxSizing: "border-box", marginTop: 10 }}
              value={form[openTextField.extraField] || ""}
              onChange={(e) => update(openTextField.extraField, e.target.value)}
              placeholder={openTextField.extraLabel}
            />
          )}
        </GatePopupShell>
      )}
...
export function GateFields({
  ...
  suppressUrlFor,
  pitchingInfoTicket,
  onSendPitchingInfoTicket,
  ticketMap,
  sonyPublishMetaReady,
  artistProfileArtistTags,
  artistProfileDraft,
  onArtistProfileDraftChange,
  artistProfileExistingKeys,
  // Round 415 (Phase 2) — Profile Management's shared platform picker +
  // per-action draft, threaded down into ArtistProfileVerifyPopup the same
  // way Artist Verification's draft already is above.
  managementPlatforms,
  ...
  coTrongNetDraft,
  onCoTrongNetChange,
  onSendCoTrongNetYoutube,
  coTrongNetSent,
  youtubeAdsUrl,
  youtubeBookingNote,
  onChangeYoutubeAdsUrl,
  onChangeYoutubeBookingNote,
  publishingHdLocked,
  sharedLabels,
}) {
  ...
  return (
    <div>
      ...
      {form.gate_artist_profile_verify === "true" && onArtistProfileDraftChange && (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 12, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <div style={{ fontSize: 12, color: "var(--text-faint)" }}>
            {(artistProfileArtistTags || []).length === 0
              ? "No Main/Feature Artist tags yet — add one above first."
              : (() => {
                  // Round 414 — count of artists with at least one type
                  // checked, across both cards (not a full completeness
                  // check — that's the popup's own inline validation).
                  const checkedCount = ARTIST_VERIFY_CARD_TYPES.reduce(
                    (sum, type) => sum + Object.values(artistProfileDraft?.[type] || {}).filter((row) => row.checked).length,
                    0
                  );
                  return checkedCount > 0 ? `${checkedCount} artist request(s) in progress.` : "Verify or set up which artist(s)?";
                })()}
          </div>
      ...
      {showArtistProfilePopup && onArtistProfileDraftChange && (
        <ArtistProfileVerifyPopup
          styles={styles}
          artistTags={artistProfileArtistTags || []}
          draft={artistProfileDraft}
          onDraftChange={onArtistProfileDraftChange}
          existingKeys={artistProfileExistingKeys || new Set()}
              ...
              {entries.map((entry, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
                  ...
                  <div className={styles.field} style={{ marginBottom: 0, flex: 1, minWidth: 140 }}>
                    ...
                    <SharedLabelInput
                      styles={styles}
                      value={entry.shared_label}
                      onChange={(v) => updateEntry(i, "shared_label", v)}
                      options={sharedLabels}
                    />
                  </div>
