...
export const TEAM_TICKET_TYPES = {
  OPS: [
    ...
    // Round 280 — Bổ Sung DATA, requester side (OPS picks the release,
    // AR is the executor who resolves the missing Metadata Checklist
    // fields — see app/tickets/bo-sung-data/page.js).
    "bo_sung_data",
    // Round 290 — Publishing, per explicit request ("add ops team as
    // executor beside legal"). OPS's Tickets switcher/index now lists it,
    // and app/tickets/publishing/page.js's PIC dropdown now offers OPS
    // profiles alongside Legal's. Note: ticket_tabs.executor_team (the
    // column the new-ticket notification trigger reads — see this file's
    // top comment) is a single text value and still reads 'Legal' only
    // for this type, so a new Publishing ticket still only pings Legal —
    // extending that to both teams would need its own schema change
    // (executor_team -> text[], or a join table), not done here since it
  ...
  AR: [
    ...
    "stream_update",
    // Round 280 — Bổ Sung DATA, executor side.
    "bo_sung_data",
  ],
...
export const TICKET_TYPE_LABELS = {
  ...
  // Round 280 — see app/tickets/bo-sung-data/page.js.
  bo_sung_data: "Bổ Sung DATA",
};
...
export const TICKET_ROUTES = {
  ...
  // Round 280 — see app/tickets/bo-sung-data/page.js.
  bo_sung_data: "/tickets/bo-sung-data",
};
...
// Round 315 — "cost_mkt" (Cost Marketing) added, Marketing-only per the
// reference sheet's own title ("WORKSTATION COST MKT") and this being a
// cost-tracking layer over Booking Board data Marketing already owns.
export const TEAM_WORKSTATION_TYPES = {
  Marketing: ["booking", "package_price", "milestone", "cost_mkt"],
