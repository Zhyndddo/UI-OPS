...
import { COPYRIGHT_ITEMS, normalizeCopyrightChecklist, copyrightEntryIsComplete } from "./copyrightChecklist";
...
export default function CopyrightRightsPopup({ trackLabel, checklist, onClose, onSave }) {
  ...
  const [activeKey, setActiveKey] = useState(
    // Round 165 fix — used to default to the first not-YET-DECLARED tab
    // (Owner blank), which meant a right with Owner filled in but Contract
    // or Validity Period still missing was treated as "done" and skipped
    // over. Send Upload's real gate needs all 3 FULLY complete, so that's
    // exactly the tab this popup should jump to — otherwise re-opening
    // "Edit Rights" lands back on an already-finished tab while the actual
    // gap (a filled-Owner-but-missing-Contract right) sits on a different
    // tab nobody's pointed you to, which was the root cause of "AR filled
    // everything, Send Upload still says Copyright Checklist incomplete."
    // Falls back to the first tab if every right is genuinely complete.
    COPYRIGHT_ITEMS.find((item) => !copyrightEntryIsComplete(normalized[item.key]))?.key || COPYRIGHT_ITEMS[0].key
  );
