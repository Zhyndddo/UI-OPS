...
import { COPYRIGHT_ITEMS, COPYRIGHT_TYPE_OPTIONS, COPYRIGHT_CONTRACT_OPTIONS, emptyCopyrightChecklist, normalizeCopyrightChecklist } from "./copyrightChecklist";
...
// Copyright Checklist is flattened into 5 columns per item (type/subtype/
// subtype name/contract type/contract text) — 15 columns total across the
// 3 items. Re-assembled into the nested jsonb shape on import.
function copyrightFields() {
  const out = [];
  COPYRIGHT_ITEMS.forEach((item) => {
    out.push({ key: `copyright.${item.key}.type`, header: `${item.label} — Loại`, kind: "choice", options: COPYRIGHT_TYPE_OPTIONS });
    out.push({ key: `copyright.${item.key}.subtype`, header: `${item.label} — Đối tác (nếu Hợp tác Độc quyền)`, kind: "choice", options: item.subOptions });
    out.push({ key: `copyright.${item.key}.subtypeName`, header: `${item.label} — Tên đối tác`, kind: "text" });
    out.push({ key: `copyright.${item.key}.contractType`, header: `${item.label} — Có hợp đồng`, kind: "choice", options: COPYRIGHT_CONTRACT_OPTIONS });
    out.push({ key: `copyright.${item.key}.contractText`, header: `${item.label} — Chi tiết hợp đồng`, kind: "text" });
  });
  return out;
}
