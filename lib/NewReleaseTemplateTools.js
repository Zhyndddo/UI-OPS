...
import { COPYRIGHT_ITEMS, emptyCopyrightChecklist, normalizeCopyrightChecklist } from "./copyrightChecklist";
...
// Copyright Checklist (flat 3-field template, 2nd follow-up) is flattened
// into 4 columns per item — Owner / Validity From / Validity To / Contract
// — 12 columns total across the 3 rights. Re-assembled into the nested
// jsonb shape on import. "Không thời hạn" in the Valid To column
// (case-insensitive) sets noTimeLimit:true instead of trying to parse it
// as a date.
function copyrightFields() {
  const out = [];
  COPYRIGHT_ITEMS.forEach((item) => {
    out.push({ key: `copyright.${item.key}.owner`, header: `${item.label} — Owner`, kind: "text" });
    out.push({ key: `copyright.${item.key}.validFrom`, header: `${item.label} — Validity From`, kind: "date" });
    out.push({ key: `copyright.${item.key}.validTo`, header: `${item.label} — Validity To (hoặc "Không thời hạn")`, kind: "dateOrNoLimit" });
    out.push({ key: `copyright.${item.key}.contract`, header: `${item.label} — Contract`, kind: "text" });
  });
  return out;
}

const NO_TIME_LIMIT_WORDS = ["không thời hạn", "khong thoi han", "no time limit", "vĩnh viễn", "vinh vien", "perpetual"];
...
export async function downloadNewReleaseTemplate() {
  ...
  const exampleRow = TEMPLATE_FIELDS.map((f) => {
    ...
    if (f.kind === "dateOrNoLimit") return "Không thời hạn";
...
export function parseNewReleaseTemplateRows(rows) {
  ...
  TEMPLATE_FIELDS.forEach((f) => {
    ...
    if (f.key.startsWith("copyright.")) {
      ...
      if (noTimeLimitFlag !== null) copyrightPatch[itemKey].noTimeLimit = noTimeLimitFlag;
    } else {
