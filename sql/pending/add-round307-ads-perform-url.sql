...
--
-- SUPERSEDED by Round 308 (add-round308-rename-promotion-package-to-ads-
-- perform.sql): per immediate follow-up correction, Ads Perform and
-- Promotion Package turned out to be meant as ONE field (renamed), not
-- two. This file is kept for history / in case only this one ran before
-- the correction landed — Round 308's migration is safe to run either
-- way (it creates ads_perform_url itself if this file never ran). Run
-- Round 308 either way; running this one first or not doesn't matter.
--
