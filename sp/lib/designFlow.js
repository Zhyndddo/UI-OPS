...
// add a note"). Round 80 — CANCEL joined this list too, matching the new
// global "refund/cancel moves need a reason" rule (Design has no REFUND
// state of its own, CANCEL is its only terminal "didn't happen" state, so
// it's the one that needs to pick up this behavior here).
export const NOTE_REQUIRED_STATUSES = ["PENDING", "REVISE", "CANCEL"];
