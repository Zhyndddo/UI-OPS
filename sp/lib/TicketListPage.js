...
export default function TicketListPage({ typeKey, basePath, externalLink }) {
  ...
  async function load() {
    ...
    if (releaseOverride) {
      const dids = [...new Set((data || []).map((t) => t.data?.[releaseOverride.fieldKey]).filter(Boolean))];
      if (dids.length > 0) {
        const { data: rels } = await supabase.from("releases").select(`id, did, ${releaseOverride.selectField}`).in("did", dids);
        const map = {};
