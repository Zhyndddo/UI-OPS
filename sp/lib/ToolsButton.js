...
import { buildNewReleasePreviewNote } from "./newReleasePreviewNote";
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
      {bucket.tools.map((t) =>
        ...
        ) : t.generator === "newReleasePreviewNote" ? (
          <NewReleasePreviewButton key={t.key || t.label} tool={t} btnStyle={btnStyle} />
        ) : t.url ? (
...
// Round 229 — "New Release Preview" (round 155 item 3, lib/
// newReleasePreviewNote.js) is a generator like Zing above, but it picks
// a release via search instead of pasting DIDs (same search-and-pick flow
// as app/tool-directory/page.js's NewReleasePreviewCard, ported here so
// New Release Setup's own topbar row has the same tool, not just the
// standalone Tools Directory page).
function NewReleasePreviewButton({ tool, btnStyle }) {
  const [open, setOpen] = useState(false);
      ...
      {open && (
        <div
          ...
          style={{
            position: "absolute", top: "100%", left: 0, marginTop: 6, zIndex: 400,
            background: "var(--bg-card)", border: "1px solid var(--border-strong)", borderRadius: 8,
            boxShadow: "0 8px 24px rgba(0,0,0,0.3)", width: 280, padding: 10,
          }}
        >
          <input
          ...
          />
          {loading && <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>Searching…</div>}
          {results.length > 0 && (
