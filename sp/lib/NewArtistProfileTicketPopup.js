...
import { REQUEST_TYPES, fieldsForType, platformOptionsForType, mergeNamesMismatch } from "./artistProfileRequestTypes";
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Round 196 — non-blocking warning only, per explicit request
              ("send a warning instead of blocking outright"): doesn't
              touch `error`/submitting at all, so a name mismatch never
              stops the ticket from being created. */}
          {requestType === "merge" && mergeNamesMismatch(form.wrongProfileName, form.correctProfileName) && (
            <div style={{ fontSize: 11, color: "#ffca4d", background: "rgba(255,202,77,0.1)", border: "1px solid #5a4a1a", borderRadius: 6, padding: "8px 10px", marginBottom: 12 }}>
