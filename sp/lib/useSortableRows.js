...
// Round 224 — initialSort (optional, backward-compatible) lets a caller
// restore a remembered { key, dir } instead of always starting at the
// default sort — see app/releases/page.js's sessionStorage-backed
// "remember position". Every existing caller that doesn't pass it
// behaves exactly as before.
export function useSortableRows(rows, options = {}) {
  const { defaultDateKey = "release_date", dateKeys = [defaultDateKey], initialSort = null } = options;
  const [sort, setSort] = useState(initialSort); // null = default sort | { key, dir: "asc" | "desc" }
  ...
  // setSort (raw setter, Round 224) — for a caller that needs to restore
  // a remembered { key, dir } post-mount (see app/releases/page.js) without
  // replaying it through toggleSort's cycle logic. Additive; every
  // existing caller ignores it same as any other unused destructure.
  return { sorted, sort, setSort, toggleSort, resetSort, isDefault: sort === null };
}
